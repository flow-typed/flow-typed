...
type Radium$MediaQueryListListener = (mql: Radium$MediaQueryList) => mixed;

type Radium$MediaQueryList = {
  matches: bool;
  addListener(listener: Radium$MediaQueryListListener): void;
  removeListener(listener: Radium$MediaQueryListListener): void;
};

type Radium$Plugin = (pluginConfig: Radium$PluginConfig) => Radium$PluginResult;
type Radium$MatchMediaType = (mediaQueryString: string) => Radium$MediaQueryList;

type Radium$RadiumConfig = {
  matchMedia?: Radium$MatchMediaType,
  plugins?: Array<Radium$Plugin>,
  userAgent?: string,
};
...
declare module 'radium' {
  declare class Radium {
    ...
    <T: ReactClass>(config: Radium$RadiumConfig): (reactClass: T) => T;
    keyframes(animationObject: Object, name?: string): string;
    Style: React.Component<{scopeSelector: string}, {config?: ?Radium$RadiumConfig, rules: Object, scopeSelector?: ?string}>;
