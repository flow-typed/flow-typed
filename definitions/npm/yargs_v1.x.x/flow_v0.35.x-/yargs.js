...
declare module 'yargs' {
	declare type SyncCompletionFunction = (current: string, argv: any) => string[];

	declare type AsyncCompletionFunction = (current: string, argv: any, done: (completion: string[]) => void) => void;
	...
	declare interface Argv {
		...
		command(
		command: string, description: string, builder: {
		...
		command(
		command: string, description: string, builder: (args: Argv) => Options, handler: (args: Argv) => void
	): Argv,
	...
	declare interface RequireDirectoryOptions {
		recurse?: boolean,
		extensions?: string[],
