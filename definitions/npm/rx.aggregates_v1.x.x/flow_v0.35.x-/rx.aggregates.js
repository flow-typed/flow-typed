...
declare module 'npm$namespace$Rx' {
		...
		sum(
		keySelector?: (value: T, index: number, source: Observable<T>) => number, thisArg?: any
	): Observable<number>,
		minBy<TKey>(
		keySelector: (item: T) => TKey, comparer: (value1: TKey, value2: TKey) => number
	): Observable<T>,
		...
		second: Observable<TOther>, comparer: (value1: T, value2: TOther) => number
	): Observable<boolean>,
		sequenceEqual<TOther>(
		second: IPromise<TOther>, comparer: (value1: T, value2: TOther) => number
	): Observable<boolean>,
		sequenceEqual(second: Observable<T>): Observable<boolean>,
		sequenceEqual(second: IPromise<T>): Observable<boolean>,
		sequenceEqual<TOther>(
		second: TOther[], comparer: (value1: T, value2: TOther) => number
	): Observable<boolean>,
