...
alasql.fn.MYFUNC = function(x){};
alasql.aggr.MYAGGR = function(a,b,c){};

asNumber = alasql('value of select 5');
asArray = alasql('select 5 as a');
...
alasql.promise('value of select 5').then().catch();

alasql.promise(['value of select 5', ['select ? as y',7]]).then().catch();

alasql(['value of select 5']).then().catch();
