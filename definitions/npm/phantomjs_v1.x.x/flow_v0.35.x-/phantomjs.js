...
declare module 'phantomjs' {
	...
	declare interface WebPage {
		...
		open(
		url: string, method: string, data: any, callback: (status: string) => any
	): void,
		...
		sendEvent(
		mouseEventType: string, mouseX?: number, mouseY?: number, button?: string
	): void,
		sendEvent(
		keyboardEventType: string, keyOrKeys: any, aNull?: any, bNull?: any, modifier?: number
	): void,
	...
	declare interface WebServer {
		port: number,
		listen(
		port: number, cb?: (request: WebServerRequest, response: WebServerResponse) => void
	): boolean,
		listen(
		ipAddressPort: string, cb?: (request: WebServerRequest, response: WebServerResponse) => void
	): boolean,
		close(): void
	}
