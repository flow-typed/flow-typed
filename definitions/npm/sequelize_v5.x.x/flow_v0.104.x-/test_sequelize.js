...
let GUser: typeof GUserModel = s.define('user', {
...
let GTask: typeof GTaskModel = s.define('task', {
...
let WarehouseBranch: typeof WarehouseBranchInstance = s.define(
  'warehouseBranch',
  {}
);
...
const WarehouseBranches = Warehouse.belongsToMany(Branch, {
  through: WarehouseBranch,
});
const BranchWarehouses = Branch.belongsToMany(Warehouse, {
  through: WarehouseBranch,
});
...
WarehouseProducts.get([warehouse], { scope: 'baz' }).then(
  (products: { [id: number]: ProductInstance, ... }) => {}
);
WarehouseProducts.count(warehouse).then((count: number) => {});
WarehouseProducts.count(warehouse, { scope: 'baz' }).then(
  (count: number) => {}
);
WarehouseProducts.has(warehouse, [product, 2]).then((result: boolean) => {});
WarehouseProducts.has(warehouse, [product, 2], { scope: 'baz' }).then(
  (result: boolean) => {}
);
WarehouseProducts.set(warehouse, [product, 2]).then(
  (result: WarehouseInstance) => {}
);
WarehouseProducts.set(warehouse, [product, 2], {
  where: {},
  scope: 'baz',
}).then((result: WarehouseInstance) => {});
WarehouseProducts.remove(warehouse, [product, 2]).then(
  (result: typeof WarehouseProducts) => {}
);
...
WarehouseBranches.get(warehouse, { scope: 'baz' }).then(
  (branches: Array<BranchInstance>) => {}
);
WarehouseBranches.count(warehouse).then((count: number) => {});
WarehouseBranches.count(warehouse, { scope: 'baz' }).then(
  (count: number) => {}
);
WarehouseBranches.has(warehouse, [branch, 2]).then((result: boolean) => {});
WarehouseBranches.has(warehouse, [branch, 2], { scope: 'blah' }).then(
  (result: boolean) => {}
);
WarehouseBranches.set(warehouse, [branch, 2]).then((result: any) => {});
WarehouseBranches.set(warehouse, [branch, 2], { scope: 'blah' }).then(
  (result: any) => {}
);
...
ProductBarcode.get(product).then((barcode: BarcodeInstance) => {});
ProductBarcode.get([product]).then(
  (barcodes: { [id: number]: BarcodeInstance, ... }) => {}
);
...
ProductWarehouse.get([product]).then(
  (warehouses: { [id: number]: WarehouseInstance, ... }) => {}
);
...
Warehouse.findAll({ include: [{ association: WarehouseProducts }] }).then(
  (warehouses: Array<WarehouseInstance>) => {}
);
Warehouse.findAll({ include: [{ association: WarehouseBranches }] }).then(
  (warehouses: Array<WarehouseInstance>) => {}
);
Branch.findAll({ include: [{ association: BranchWarehouses }] }).then(
  (branch: Array<BranchInstance>) => {}
);
Product.findOne({ include: [{ association: ProductWarehouse }] }).then(
  (product: ?Product) => {}
);
...
declare class BranchInstance extends Model<BranchAttributes> {
  ...
  setWarehouses: BelongsToManySetMany<
    WarehouseInstance,
    number,
    WarehouseBranchAttributes
  >;
  addWarehouses: BelongsToManyAddMany<
    WarehouseInstance,
    number,
    WarehouseBranchAttributes
  >;
  addWarehouse: BelongsToManyAddOne<
    WarehouseInstance,
    number,
    WarehouseBranchAttributes
  >;
  createWarehouse: BelongsToManyCreateOne<
    WarehouseAttributes,
    WarehouseInstance,
    WarehouseBranchAttributes
  >;
  ...
  createCustomer: BelongsToManyCreateOne<
    CustomerAttributes,
    CustomerInstance,
    void
  >;
...
new Sequelize.Error();
new Sequelize.ValidationError('message');
new Sequelize.ValidationErrorItem('invalid', 'type', 'first_name', 'value');
new Sequelize.ValidationErrorItem('invalid', 'type', 'last_name', 'value');
new Sequelize.DatabaseError(new Error('original database error message'));
new Sequelize.ConnectionError(new Error('original connection error message'));
new Sequelize.ConnectionRefusedError(
  new Error('original connection error message')
);
new Sequelize.AccessDeniedError(new Error('original connection error message'));
new Sequelize.HostNotFoundError(new Error('original connection error message'));
new Sequelize.HostNotReachableError(
  new Error('original connection error message')
);
new Sequelize.InvalidConnectionError(
  new Error('original connection error message')
);
new Sequelize.ConnectionTimedOutError(
  new Error('original connection error message')
);
new Sequelize.EmptyResultError();

const uniqueConstraintError: Sequelize.ValidationError = new Sequelize.UniqueConstraintError(
  {}
);
...
User.addHook('afterCreate', 'myHook', function(
  instance: Model<any>,
  options: Object,
  next: Function
) {
...
User.beforeUpsert('myHook', function(user, options) {
  throw new Error('Whoops');
});

User.afterUpsert(function(result, options) {
  throw new Error('Whoops');
});
User.afterUpsert('myHook', function(result, options) {
  throw new Error('Whoops');
});
...
user
  .update({ username: 'bar' }, { where: { username: 'foo' }, transaction: t })
  .then(p => p);
user.update({ a: 3 }).then(p => p);
user.update({ a: 3 }, { fields: ['secretValue'], logging: function() {} });
...
User.findAll({
  include: [{ model: User, include: [User, { model: User, as: 'residents' }] }],
});
User.findAll({
...
User.findAll({
  include: [User],
  order: [['id', 'ASC NULLS LAST'], [User, 'id', 'DESC NULLS FIRST']],
});
User.findAll({
  include: [
...
User.findOne({
  include: [{ model: User, as: 'ToDo' }, { model: User, as: 'DoTo' }],
});
 ...
 */
User.findOne({ where: { intVal: { gt: 5 } } });
User.findOne({ where: { intVal: { lte: 5 } } });
...
User.create(
  { id: 1, title: 'e', Tags: [{ id: 1, name: 'c' }, { id: 2, name: 'd' }] },
  { include: [User] }
);
User.create({ id: 'My own ID!' }).then(i => i.isNewRecord);

let findOrRetVal: Promise<[Model<Object>, boolean]>;
findOrRetVal = User.findOrBuild({ where: { username: 'foo' } });
findOrRetVal = User.findOrBuild({
  where: { username: 'foo' },
  transaction: t,
});
findOrRetVal = User.findOrBuild({
...
User.bulkCreate([{ a: 'b', c: 'd', e: 'f' }, { a: 'b', c: 'd', e: 'f' }], {
  fields: ['a', 'b'],
});
User.bulkCreate(
...
User.bulkCreate([{ name: 'a', c: 'b' }, { name: 'e', c: 'f' }], {
  fields: ['e', 'f'],
...
User.unscoped().findOne({ where: { username: 'bob' } });
User.unscoped().count();
...
let autoCallback = (t: Transaction): Promise<string> => {
  return Promise.resolve('a');
};
let callbackValidator = (r: string) => {};
s.transaction(autoCallback).then(callbackValidator);
s.transaction({ isolationLevel: 'SERIALIZABLE' }, autoCallback).then(
  callbackValidator
);
s.transaction(
  { isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.SERIALIZABLE },
  autoCallback
).then(callbackValidator);
s.transaction(
  { isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.READ_COMMITTED },
  autoCallback
).then(callbackValidator);
...
s.transaction({ type: Sequelize.Transaction.TYPES.DEFERRED }, t =>
  Promise.resolve()
);
s.transaction({ type: Sequelize.Transaction.TYPES.IMMEDIATE }, t =>
  Promise.resolve()
);
s.transaction({ type: Sequelize.Transaction.TYPES.EXCLUSIVE }, t =>
  Promise.resolve()
);
