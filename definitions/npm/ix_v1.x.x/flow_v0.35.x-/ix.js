...
declare module 'npm$namespace$Ix' {
	...
	declare export interface Enumerable<T> {
		...
		distinctBy<TKey>(
		keySelector: (item: T) => TKey, comparer?: EqualityComparer<TKey, TKey>
	): Enumerable<T>,
		...
		scan<TAccumulate>(
		seed: TAccumulate, accumulate: (acc: TAccumulate, item: T) => TAccumulate
	...
	declare export interface EnumerableStatic {
		...
		initialState: TState, condition: Predicate<TState>, iterate: (state: TState) => TState, resultSelector: (state: TState) => TResult
	): Enumerable<TResult>,
		using<TResource, T>(
		resourceFactory: () => TResource, enumerableFactory: (resource: TResource) => Enumerable<T>
	): Enumerable<T>,
		...
		while<T>(
		condition: EnumerablePredicate<Enumerable<T>>, source: Enumerable<T>
	): Enumerable<T>,
		whileDo<T>(
		condition: EnumerablePredicate<Enumerable<T>>, source: Enumerable<T>
	): Enumerable<T>,
		if<T>(
		condition: () => boolean, thenSource: Enumerable<T>, elseSource?: Enumerable<T>
	): Enumerable<T>,
		ifThen<T>(
		condition: () => boolean, thenSource: Enumerable<T>, elseSource?: Enumerable<T>
	): Enumerable<T>,
		doWhile<T>(
		source: Enumerable<T>, condition: EnumerablePredicate<Enumerable<T>>
	): Enumerable<T>,
		case<T>(
		selector: () => number, sources: {
		[key: number]: Enumerable<T>
	}, defaultSource?: Enumerable<T>
	): Enumerable<T>,
		case<T>(
		selector: () => string, sources: {
		[key: string]: Enumerable<T>
	}, defaultSource?: Enumerable<T>
	): Enumerable<T>,
		switchCase<T>(
		selector: () => number, sources: {
		...
		for<T, TResult>(
		source: Enumerable<T>, resultSelector: EnumerableFunc<T, TResult>
	): Enumerable<TResult>,
		forIn<T, TResult>(
		source: Enumerable<T>, resultSelector: EnumerableFunc<T, TResult>
	): Enumerable<TResult>
