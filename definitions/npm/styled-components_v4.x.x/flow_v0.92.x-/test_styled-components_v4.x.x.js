...
describe('styled', () => {
  type InstrinsicAbstractComponent<InstrinsicElement> = React.AbstractComponent<React.ElementConfig<InstrinsicElement>, InstrinsicElement>;


  it('should map to correct element', () => {
    const Span1 : InstrinsicAbstractComponent<'span'> = styled.span``
    const Span2 : InstrinsicAbstractComponent<'span'> = styled('span')``
  })

  it('should\'t map incorrectly', () => {
    // $ExpectError - Wrong component type
    const Div1 : InstrinsicAbstractComponent<'span'> = styled.div``
    // $ExpectError - Wrong component type
    const Div2 : InstrinsicAbstractComponent<'span'> = styled('div')``
  })

  it('renders as the correct element', () => {
    const Span1 : InstrinsicAbstractComponent<'span'> = styled.span``
    const Span2 : InstrinsicAbstractComponent<'span'> = styled('span')``
    const Div : InstrinsicAbstractComponent<'div'> = styled('div')``

    const span1 : React.Element<InstrinsicAbstractComponent<'span'>> = <Span1 />
    const span2 : React.Element<InstrinsicAbstractComponent<'span'>> = <Span2 />

    // $ExpectError - Wrong element type
    const div3 : React.Element<InstrinsicAbstractComponent<'span'>> = <Div />
  })
  ...
  it('should understand interpolations', () => {
      type Props = { className: string, color: string }

      const Hello = (p : Props ) => null

      const StyledHello = styled(Hello)`
        color: ${props => props.theme.red};
      `

      // $ExpectError - blue does not exist on theme
      const StyledHello2 = styled<{|red: string|}, *>(Hello)`
        color: ${props => props.theme.blue};
      `

      const StyledHello3 = styled(Hello)`
        color: ${props => props.color};
      `

      // $ExpectError - name is not a prop
      const StyledHello4 = styled(Hello)`
...
describe('wrapping functional component', () => {
  ...
  type StyledHelloExpectedType = React.Element<React.AbstractComponent<{...$Exact<Props>, className?: string}>>

  it('understands props that the wrapped component wants', () => {
    const hello1 : StyledHelloExpectedType = <StyledHello name="World" />

    // $ExpectError - Invalid prop type
    const hello2 : StyledHelloExpectedType = <StyledHello name={3} />
  })
})
...
describe('attrs API', () => {
  ...
  it('errors when props are missing', () => {
    ...
    const hello = <StyledHello foo="bar" />
  })
})
