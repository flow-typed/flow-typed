...
import type { $AxiosXHR, CancelTokenSource, Cancel, AxiosPromise } from 'axios';
...
axios
  .all([
    extended.specialPut('foo').then(r => {
      // $ExpectError
      (r.statusText: number);
    }),
    Promise.reject(12),
...
const promise1: AxiosPromise<{ foo: string }, { bar: string }> = axios({
  url: '/',
  method: 'post',
  data: { foo: 'bar' },
});
...
const promise2: AxiosPromise<{ foo: number }> = axios({
  url: '/',
  method: 'post',
  data: { foo: 1 },
});
