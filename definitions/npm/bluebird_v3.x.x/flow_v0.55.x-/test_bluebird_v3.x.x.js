...
Bluebird.all([new Bluebird(() => {})]);
Bluebird.map([1], (x: number) => new Bluebird(() => {})).all();
Bluebird.map(
  Bluebird.resolve([1]),
  (x: number) => new Bluebird(() => {})
).all();
Bluebird.reject("test");
Bluebird.all([1]);
Bluebird.all(Promise.resolve([new Bluebird(() => {})]));
...
Bluebird.all([1, Bluebird.resolve(1), Promise.resolve(1)]).then(function(
  r: Array<number>
) {});
Bluebird.all([
  "hello",
  Bluebird.resolve("world"),
  Promise.resolve("!")
]).then(function(r: Array<string>) {});

Bluebird.join(1, Bluebird.resolve(2), function(a, b) {
  return a + b;
}).then(function(s) {
  return s + 1;
});
Bluebird.join(1, Bluebird.resolve(2), Promise.resolve(3), function(a, b) {
  return a + b;
}).then(function(s) {
  return s + 1;
});
...
Bluebird.reduce(
  [5, Bluebird.resolve(6), Promise.resolve(7)],
  (memo, next) => memo + next
);
Bluebird.reduce(
  [5, Bluebird.resolve(6), Promise.resolve(7)],
...
Bluebird.reduce(
  Promise.resolve([5, Bluebird.resolve(6), Promise.resolve(7)]),
  (memo, next) => memo + next
);
Bluebird.reduce(
  Bluebird.resolve([5, Bluebird.resolve(6), Promise.resolve(7)]),
  (memo, next) => memo + next,
  1
);

Bluebird.reduce([1, Bluebird.resolve(2), Promise.resolve(3)], (prev, num) =>
  Promise.resolve(prev * num)
);
Bluebird.reduce([1, Bluebird.resolve(2), Promise.resolve(3)], (prev, num) =>
  Bluebird.resolve(prev * num)
);
//$ExpectError
Bluebird.reduce(
  [1, Bluebird.resolve(2), Promise.resolve(3)],
  (prev, num) => Bluebird.resolve(prev * num),
  "hello"
);
...
// $ExpectError Needs to be re-implemented
Bluebird.bind(() => {});
