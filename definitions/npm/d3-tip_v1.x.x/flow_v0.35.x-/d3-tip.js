...
declare module 'npm$namespace$d3' {
	declare type TooltipDirection = ("n" | "s" | "e" | "w" | "nw" | "ne" | "sw" | "se");
	...
	declare interface Tooltip {
		...
		offset<Datum>(
		func: (datum: Datum, index: number, outerIndex: number) => [number, number]
	): Tooltip,
		direction(): TooltipDirection,
		direction(direction: TooltipDirection): Tooltip,
		direction<Datum>(
		func: (datum: Datum, index: number, outerIndex: number) => TooltipDirection
	): Tooltip,
		html(): string,
	...
	declare export function tip(): Tooltip
