...
declare module 'kefir' {
	...
	declare export interface Observer<T, S> {
		value?: (value: T) => void,
		error?: (error: S) => void,
		end?: () => void
	}

	declare export interface Observable<T, S> {
		...
		observe(
		onValue?: (value: T) => void, onError?: (error: S) => void, onEnd?: () => void
	): Subscription
	}

	declare export interface Stream<T, S> {
		...
		bufferWithCount(count: number, options?: {
		flushOnEnd: boolean
	}): Stream<T[], S>,
		bufferWithTimeOrCount(
		interval: number, count: number, options?: {
		flushOnEnd: boolean
	}
	): Stream<T[], S>,
		...
		combine<U, V, W>(
		otherObs: Stream<U, V>, combinator?: (value: T, ...values: U[]) => W
	): Stream<W, S | V>,
		zip<U, V, W>(
		otherObs: Stream<U, V>, combinator?: (value: T, ...values: U[]) => W
	): Stream<W, S | V>,
		...
		sampledBy<U, V, W>(
		otherObs: Observable<U, V>, combinator?: (a: T, b: U) => W
	): Stream<W, S>,
		skipUntilBy<U, V>(otherObs: Observable<U, V>): Stream<U, V>,
		takeUntilBy<U, V>(otherObs: Observable<U, V>): Stream<U, V>,
		bufferBy<U, V>(
		otherObs: Observable<U, V>, options?: {
		flushOnEnd: boolean
	}
	): Stream<T[], S>,
		bufferWhileBy<U>(
		otherObs: Observable<boolean, U>, options?: {
		flushOnEnd?: boolean,
	...
	declare export interface Property<T, S> {
		changes(): Stream<T, S>,
		...
		withHandler<U, V>(
		handler: (emitter: Emitter<T, S>, event: Event<T | S>) => void
	): Property<U, S>,
		combine<U, V, W>(
		otherObs: Property<U, V>, combinator?: (value: T, ...values: U[]) => W
	): Property<W, S | V>,
		zip<U, V, W>(
		otherObs: Property<U, V>, combinator?: (value: T, ...values: U[]) => W
	): Property<W, S | V>,
	...
	declare export interface ObservablePool<T, S> {
	...
	declare export interface Emitter<T, S> {
		emit(value: T): void,
		error(error: S): void,
		end(): void,
		emitEvent(event: {
	...
	declare export function withInterval<T, S>(
		interval: number, handler: (emitter: Emitter<T, S>) => void
	): Stream<T, S>
	...
	declare export function fromEvents<T, S>(
		target: EventTarget | NodeJS.EventEmitter | {
		on: Function,
		off: Function
	}, eventName: string, transform?: (value: T) => S
	): Stream<T, S>
	...
	declare export function combine<T, S, U>(
		obss: Observable<T, S>[], passiveObss: Observable<T, S>[], combinator?: (...values: T[]) => U
	): Observable<U, S>

	declare export function combine<T, S, U>(
		obss: Observable<T, S>[], combinator?: (...values: T[]) => U
	): Observable<U, S>

	declare export function zip<T, S, U>(
		obss: Observable<T, S>[], passiveObss?: Observable<T, S>[], combinator?: (...values: T[]) => U
	): Observable<U, S>
