import type { Quill, DeltaStatic } from 'npm$namespace$Quill'

declare module 'quill' {
					declare module.exports: Quill


}

declare module 'npm$namespace$Quill' {
	declare type Key = {
		key: string,
		shortKey?: boolean
	};
	...
	declare export interface KeyboardStatic {
		...
		addBinding(
		key: Key, context: any, callback: (range: RangeStatic, context: any) => void
	): void
	}

	declare export interface ClipboardStatic {
		addMatcher(
		selector: string, callback: (node: any, delta: DeltaStatic) => DeltaStatic
	): void,
		addMatcher(
		nodeType: number, callback: (node: any, delta: DeltaStatic) => DeltaStatic
	): void,
	...
	declare export interface DeltaStatic {
	...
	declare export interface RangeStatic {
		new (): RangeStatic,
		index: number,
		length: number
	}

	declare export interface Quill {
		...
		insertText(
		index: number, text: string, format: string, value: any, source?: Sources
	): DeltaStatic,
		...
		formatLine(
		index: number, length: number, format: string, value: any, source?: Sources
	): DeltaStatic,
		...
		on(
		eventName: string, callback: (<T>(delta: T, oldContents: T, source: string) => void) | ((name: string, ...args: any[]) => void)
	): Quill,
		once(
		eventName: string, callback: (delta: DeltaStatic, source: string) => void
	): Quill,
		off(
		eventName: string, callback: (delta: DeltaStatic, source: string) => void
	): Quill,
