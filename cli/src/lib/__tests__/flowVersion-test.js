...
describe('flowVersion', () => {
  describe('parseVersion', () => {
    it('parses an exact flow version, no pre-rel', () => {
      expect(parseVersion('v0.1.2', 'testContext', false)).toEqual([6, {
        major: 0,
        minor: 1,
        patch: 2,
        prerel: null,
    ...
    it('parses an exact flow version, with pre-rel', () => {
      expect(parseVersion('v0.1.2-prerel-secondpart', 'testContext', false)).toEqual([25, {
        major: 0,
        minor: 1,
        patch: 2,
    ...
    it('parses an exact flow version, no patch no pre-rel', () => {
      expect(parseVersion('v0.1', 'testContext', false)).toEqual([4, {
        major: 0,
        minor: 1,
        patch: 'x',
        prerel: null,
    ...
    it('parses an exact flow version, no patch with pre-rel', () => {
      ...
      expect(parseVersion('v0.1-ignoredprerel', 'testContext', false)).toEqual([4, {
        major: 0,
        minor: 1,
        patch: 'x',
        prerel: null,
    ...
    it('parses wildcard minor versions', () => {
      expect(parseVersion('v0.x', 'testContext', false)).toEqual([4, {
        major: 0,
        minor: 'x',
        patch: 'x',
        prerel: null,
    ...
    it('parses wildcard patch versions', () => {
      expect(parseVersion('v0.1.x', 'testContext', false)).toEqual([6, {
        major: 0,
        minor: 1,
        patch: 'x',
        prerel: null,
    ...
    it('parses prerel when range upper is possible', () => {
      expect(parseVersion('v0.1.x-prerel', 'testContext', true)).toEqual([13, {
        major: 0,
        minor: 1,
        patch: 'x',
        prerel: 'prerel',
    ...
    it('doesnt parse prerel when range upper is possible and present', () => {
      expect(parseVersion('v0.1.x-v0.1', 'testContext', true)).toEqual([6, {
        major: 0,
        minor: 1,
        patch: 'x',
        prerel: null,
    ...
    it('parses prerel on lower bound but excludes upper-range version', () => {
      expect(parseVersion('v0.1.x-prerel-prerel2-v0.1', 'testContext', true)).toEqual([21, {
        major: 0,
        minor: 1,
        patch: 'x',
